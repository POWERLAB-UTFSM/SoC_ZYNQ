...
void CPWM6B_InterruptHandler(void *CallbackRef)
{
    ...
    /* vérifier status */
    if (XCpwm6b_GetInterruptStatus(InstancePtr))
    {
        /* ACK IP */
        XCpwm6b_ClearInterrupt(InstancePtr);
    }
}
...
int INTC_Init(XIntc *IntcInstancePtr)
{
    ...
    /* 👉 DeviceId = 0 car 1 seule instance */
...
int GIC_Init(XScuGic *GicInstancePtr)
{
    ...
    status = XScuGic_Connect(GicInstancePtr,
                            INTC_IRQ_ID,
                            (Xil_InterruptHandler)XIntc_InterruptHandler,
                            &xintc_my_inst);
    if (status != XST_SUCCESS)
        return XST_FAILURE;

    XScuGic_Enable(GicInstancePtr, INTC_IRQ_ID);
...
int XCpwm6b_My_Init(XCpwm6b *InstancePtr, UINTPTR BaseAddr)
{
    ...
    XCpwm6b_WriteCarrierInit(InstancePtr, 0000);
    ...
    XCpwm6b_WriteSyncMode(InstancePtr, MAX_EVENT);
    ...
    XCpwm6b_WriteDeadTimeOnOff(InstancePtr, REG_ON);
    ...
    XCpwm6b_EnableInterrupt(InstancePtr, REG_ON);
...
int _HW_My_Init(void)
{
    ...
    status = INTC_Init(&xintc_my_inst);
    if (status != XST_SUCCESS)
        return XST_FAILURE;

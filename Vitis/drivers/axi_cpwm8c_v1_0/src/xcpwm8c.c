...
void XCpwm8c_WriteFullStatus(XCpwm8c * InstancePtr)
{
	for(u16 i_ch=0; i_ch < CPWM8C_N_CHANNEL;i_ch++){
...
void XCpwm8c_WriteCountMax(XCpwm8c *InstancePtr, u16 Channel, u16 CountMaxim)
{
	...
	if(Channel < CPWM8C_N_CHANNEL){
		...
		Xil_Out16((InstancePtr->BaseAddr) + (2*(Channel)), (u16)(CountMaxim));
	}
}

void XCpwm8c_WriteCountInit(XCpwm8c *InstancePtr, u16 Channel, u16 CountInit)
{
	Xil_AssertVoid(InstancePtr != NULL);
	Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);
	if(Channel < CPWM8C_N_CHANNEL){
		InstancePtr->ChParam[Channel].InitCarr=CountInit;
		Xil_Out16(InstancePtr->BaseAddr + CPWM8C_OFFSET_COUNTINIT + 2*(Channel), (u16)(CountInit));
	}
}
...
void XCpwm8c_WriteCompare(XCpwm8c *InstancePtr, u16 Channel, u16 Compare)
{
	...
	if(Channel < CPWM8C_N_CHANNEL){
		...
		Xil_Out16(InstancePtr->BaseAddr + CPWM8C_OFFSET_COMPARE + 2*(Channel), (u16)(Compare));
	}
}
